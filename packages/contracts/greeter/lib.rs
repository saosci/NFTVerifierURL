...
pub use self::launchpad_psp34_nft_standard::{
    LaunchPadPsp34NftStandard, LaunchPadPsp34NftStandardRef,
};
...
pub mod launchpad_psp34_nft_standard {
    use artzero_project::traits::{
        admin::*, error::Error, launchpad_manager::ArtZeroLaunchPadRef, psp34_standard::*,
    };
    ...
    #[derive(
        Clone, Debug, Ord, PartialOrd, Eq, PartialEq, Default, scale::Encode, scale::Decode,
    )]
    #[cfg_attr(feature = "std", derive(StorageLayout, scale_info::TypeInfo))]
    pub struct Phase {
        is_active: bool,
    ...
    pub struct Manager {
        ...
        pub phase_account_public_claimed_amount:
            Mapping<(AccountId, u8), u64, PhaseAccountPublicClaimedAmountKeys>,
    ...
    impl LaunchPadPsp34NftStandard {
        ...
        pub fn new(
            ...
            instance
                .grant_role(ADMINER, contract_owner)
        ...
        pub fn add_new_phase(
            ...
            if !self.has_role(ADMINER, self.env().caller())
                && self.env().caller() != self.manager.launchpad_contract_address
            {
                return Err(Error::InvalidCaller);
            }
            if self.manager.active_phase_count.checked_add(1).unwrap()
            ...
            self.manager.active_phase_count =
                self.manager.active_phase_count.checked_add(1).unwrap();
            if is_public {
                self.manager.available_token_amount = self
                    .manager
                    .available_token_amount
                    .checked_sub(public_minting_amount)
                    .unwrap();
            }
            ...
            self.manager
                .phases
                .insert(&self.manager.last_phase_id, &phase);
            Ok(())
        }
        ...
        pub fn update_whitelist(
            ...
            if self
                .manager
                .phase_whitelists_link
                .get(&(&account, &phase_id))
                .is_none()
            {
            ...
            self.manager.available_token_amount = self
                .manager
                .available_token_amount
                .checked_add(old_whitelist_amount)
                .unwrap()
                .checked_sub(whitelist_amount)
                .unwrap();
            ...
            self.manager
                .phase_whitelists_link
                .insert(&(&account, &phase_id), &whitelist);
            let mut phase = self.manager.phases.get(&phase_id).unwrap();
            phase.whitelist_amount = phase
                .whitelist_amount
                .checked_sub(old_whitelist_amount)
                .unwrap()
                .checked_add(whitelist_amount)
                .unwrap();
            phase.total_amount = phase
                .total_amount
                .checked_sub(old_whitelist_amount)
                .unwrap()
                .checked_add(whitelist_amount)
        ...
        pub fn mint(&mut self, mint_amount: u64) -> Result<(), Error> {
                ...
                if phase.is_active && current_time > phase.end_time {
                    available_amount = available_amount
                        .checked_add(
                            phase
                                .total_amount
                                .checked_sub(phase.claimed_amount)
                                .unwrap(),
                        )
                        .unwrap();
                }
            ...
            if mint_amount
                > available_amount
                    .checked_sub(self.manager.owner_claimed_amount)
                    .unwrap()
            {
                return Err(Error::InvalidInput);
            }
            for _i in 0..mint_amount {
                self.manager_psp34_standard.last_token_id = self
                    .manager_psp34_standard
                    .last_token_id
                    .checked_add(1)
                    .unwrap();
                if self
                    ._mint_to(caller, Id::U64(self.manager_psp34_standard.last_token_id))
                    .is_err()
                {
                    return Err(Error::CannotMint);
                }
            }
            self.manager.owner_claimed_amount = self
                .manager
                .owner_claimed_amount
                .checked_add(mint_amount)
                .unwrap();
            Ok(())
        }
        ...
        pub fn public_mint(&mut self, phase_id: u8, mint_amount: u64) -> Result<(), Error> {
            ...
            if (phase.start_time..=phase.end_time).contains(&current_time) {
                ...
                if phase
                    .public_claimed_amount
                    .checked_add(mint_amount)
                    .unwrap()
                    > phase.public_minting_amount
                {
                    return Err(Error::InvalidInput);
                }
                ...
                for _i in 0..mint_amount {
                    ...
                    self.manager.public_minted_count =
                        self.manager.public_minted_count.checked_add(1).unwrap();
                    if self
                        ._mint_to(caller, Id::U64(self.manager_psp34_standard.last_token_id))
                        .is_err()
                    {
                ...
                if self
                    .manager
                    .phase_account_public_claimed_amount
                    .get(&(&caller, &phase_id))
                    .is_none()
                {
                    self.manager
                        .phase_account_public_claimed_amount
                        .insert(&(&caller, &phase_id), &mint_amount);
                } else {
                    let phase_account_public_old_claimed_amount = self
                        .manager
                        .phase_account_public_claimed_amount
                        .get(&(&caller, &phase_id))
                        .unwrap();
                    self.manager.phase_account_public_claimed_amount.insert(
                        &(&caller, &phase_id),
                        &phase_account_public_old_claimed_amount
                            .checked_add(mint_amount)
                            .unwrap(),
                    );
                }

                phase.public_claimed_amount = phase
                    .public_claimed_amount
                    .checked_add(mint_amount)
                    .unwrap();
        ...
        pub fn whitelist_mint(&mut self, phase_id: u8, mint_amount: u64) -> Result<(), Error> {
            ...
            if (phase.start_time..=phase.end_time).contains(&current_time) {
                ...
                let project_mint_fee = caller_info
                    .minting_fee
                    .checked_mul(project_mint_fee_rate as u128)
                    .unwrap()
                    .checked_mul(mint_amount as u128)
                ...
                self.manager
                    .phase_whitelists_link
                    .insert(&(&caller, &phase_id), &caller_info);
        ...
        #[ink(message)]
        #[modifiers(only_role(ADMINER))]
        pub fn deactive_phase(&mut self, phase_id: u8) -> Result<(), Error> {
            if self.manager.phases.get(&phase_id).is_none() {
                return Err(Error::PhaseNotExist);
            }
            let mut phase = self.manager.phases.get(&phase_id).unwrap();
            if phase.claimed_amount != 0
                || self.manager.phase_account_link.count(phase_id) != 0
                || phase.start_time <= self.env().block_timestamp()
                || !phase.is_active
            {
            ...
            self.manager.active_phase_count =
                self.manager.active_phase_count.checked_sub(1).unwrap();
            if phase.is_public {
                self.manager.available_token_amount = self
                    .manager
                    .available_token_amount
                    .checked_add(phase.public_minting_amount)
                    .unwrap();
            }
            self.manager.phases.insert(&phase_id, &phase);
            Ok(())
        ...
        pub fn update_schedule_phase(
            ...
            if public_max_minting_amount
                > ArtZeroLaunchPadRef::get_public_max_minting_amount(
                    &self.manager.launchpad_contract_address,
                )
            {
                return Err(Error::InvalidInput);
            ...
            for index in 0..self.manager.last_phase_id {
                if index.checked_add(1).unwrap() != phase_id {
                    let phase = self.manager.phases.get(&(index + 1)).unwrap();
                    if phase.is_active
                        && ((phase.start_time..=phase.end_time).contains(&start_time)
                            || (phase.start_time..=phase.end_time).contains(&end_time))
                    {
                        return Err(Error::InvalidInput);
                    }
                }
            ...
            if phase.is_public && !is_public {
                ...
                phase.total_amount = phase
                    .total_amount
                    .checked_sub(phase.public_minting_amount)
                    .unwrap();
                ...
                phase.total_amount = phase
                    .total_amount
                    .checked_add(public_minting_amount)
                    .unwrap();
            } else if phase.is_public && is_public {
                self.manager.available_token_amount = self
                    .manager
        ...
        #[ink(message)]
        pub fn get_owner_claimed_amount(&self) -> u64 {
            self.manager.owner_claimed_amount
        }

        /// Get owner available amount
        #[ink(message)]
        pub fn get_owner_available_amount(&self) -> u64 {
            let current_time = self.env().block_timestamp();
            let mut available_amount: u64 = 0;
            for index in 0..self.manager.last_phase_id {
                let phase = self.manager.phases.get(&(index + 1)).unwrap();
        ...
        #[ink(message)]
        pub fn get_limit_phase_count(&self) -> u8 {
            self.manager.limit_phase_count
        }

        /// Get public minted count
        #[ink(message)]
        pub fn get_public_minted_count(&self) -> u64 {
            self.manager.public_minted_count
        }

        /// Get limit phase count
        #[ink(message)]
        pub fn get_project_info(&self) -> Vec<u8> {
            self.manager.project_info.clone()
        }

        /// Get Phase Schedule by Phase Id
        #[ink(message)]
        pub fn get_phase_schedule_by_id(&self, phase_id: u8) -> Option<Phase> {
        ...
        #[ink(message)]
        pub fn get_phase_account_link(&self, phase_id: u8, account_index: u64) -> AccountId {
            self.manager
                .phase_account_link
        ...
        #[ink(message)]
        pub fn get_phase_account_public_claimed_amount(
            &self,
